...
-- Requires migration 20260615045433_*.sql to have run (menus + pages already seeded).
...
FROM m, (VALUES
  ...
  ('Free Tools', NULL,                             3, true),
  ('Resources',  NULL,                             4, true),
  ('Pricing',    '/pricing',                       5, false),
  ('Comparison', '/ai-visibility-tool-comparison', 6, false),
  ('Contact',    '/contact',                       7, false)
) AS v(label, url, pos, mega);

-- Features mega children
WITH parent AS (
...
SELECT (SELECT menu_id FROM public.menu_items WHERE id = parent.id),
       parent.id, v.label, v.url, v.icon, v.descr, v.pos
FROM parent, (VALUES
  ...
  ('On-Page Optimiser',        '/on-page-optimiser',        'FileText',   'Optimise pages for AI surfaces',         3),
...
-- On-Page Optimiser nested children
WITH parent AS (
  SELECT mi.id, mi.menu_id FROM public.menu_items mi
  JOIN public.menus m ON m.id = mi.menu_id
  WHERE m.location = 'primary_nav' AND mi.label = 'On-Page Optimiser'
)
INSERT INTO public.menu_items (menu_id, parent_id, label, url, description, position)
SELECT parent.menu_id, parent.id, v.label, v.url, v.descr, v.pos
FROM parent, (VALUES
...
-- Free Tools mega children
WITH parent AS (
  SELECT mi.id, mi.menu_id FROM public.menu_items mi
  JOIN public.menus m ON m.id = mi.menu_id
  WHERE m.location = 'primary_nav' AND mi.label = 'Free Tools'
)
INSERT INTO public.menu_items (menu_id, parent_id, label, url, icon, description, position)
